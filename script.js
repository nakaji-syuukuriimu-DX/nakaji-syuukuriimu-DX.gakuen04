...
// キー用: "2026-07-02"（※UTCではなくローカル日付で作る）
...
function initDayAndTime(room) {
  ...
  if (!table) return;
  const dates = getDateArray();              // 今日〜2日後の Date
  const dateKeys = dates.map(formatKeyDate); // ["2026-07-02", "2026-07-03", ...]
  const reservations = getReservations();
  // ヘッダー行の書き換え（固定文字 → 今日〜2日後）
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const dateKey = dateKeys[i];             // 該当日付キー (例: "2026-07-04")
...
function addReservation(room, dateKey, timeSlot, user, icId) {
  ...
  const exists = list.some(r => r.icId === icId);
  if (exists) {
    alert('この日付・時間帯はすでに予約済みです。');
    return false;
  }
  list.push({
    id: Date.now(), // 簡易ID
...
function renderReservationTable(room, dateKey, timeSlot) {
  ...
  if (rows.length > 0 && rows[0].cells.length > 0) {
    ...
    bigCell.textContent = `${displayDate} ${timeSlot}`;
  }
  // ③ 予約データを表示
  ...
  for (let i = 0; i < maxDisplay; i++) {
    ...
    // 日付（"7月4日" 形式）
    cells[dateCol].textContent = displayDate;
    // 削除ボタン
...
function initReservationPage() {
  ...
  if (h1) {
    ...
    h1.textContent = `トレーニングルーム${roomNum}予約状況 `;
  }
  // ICカード入力欄追加
